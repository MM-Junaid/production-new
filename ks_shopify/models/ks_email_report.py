...
class KsEmailReport(models.Model):
    _name = 'ks.shopify.email.report'
    _rec_name = 'ks_email_conf_id'
    _description = '''Shopify Mail Report Configuration'''

    ks_email_conf_id = fields.Char(string="Email Configuration Id", readonly=True, default=lambda self: 'New')
    ks_shopify_instance_ids = fields.Many2many("ks.shopify.connector.instance", string="Shopify Instance",
                                               required=True, help="Displays Shopify Name")
    email_ids = fields.Many2many("ks.email.entry", string="Email IDs", readonly=True,
    ...
    def create(self, vals):
        ...
        seq = self.env['ir.sequence'].next_by_code('increment_my_field')
        rec.ks_email_conf_id = seq
        return rec

    @api.constrains('ks_shopify_instance_ids', 'email_ids')
    def _check_email(self):
        if not self.email_ids:
            raise ValidationError("Email Fields cannot be empty")

    @api.onchange("ks_shopify_instance_ids")
    @api.depends("ks_shopify_instance_ids", "email_ids")
    def _fill_emails_based_on_instance(self):
        ...
        """
        if self:
            if self.ks_shopify_instance_ids:
                domain = []
                for rec in self.ks_shopify_instance_ids:
    ...
    def get_template(self):
        ...
        """
        if self:
            self.email_template = self.env.ref('ks_shopify.ks_email_report_template').id
        else:
    ...
    def get_instances(self):
        ...
        """
        instances = []
        for instance in self.ks_shopify_instance_ids:
    ...
    def get_default_subject(self):
        ...
        """
        return 'Shopify Daily Sales Report [%s]' % str(date.today())
    ...
    def get_default_body(self):
        ...
        return '''
            ...
            Thank You for using Shopify Connector
            Regards,
            Shopify Connector
        '''
    ...
    def action_send_email(self):
        ...
        """
        if not self:
            records = self.env['ks.shopify.email.report'].search(
                [('active', '=', True), ('ks_shopify_instance_ids.id', '!=', False), ('email_ids.id', '!=', False)])
            for rec in records:
                template_id = rec.env.ref('ks_shopify.ks_email_report_template').id
...
class KsEmail(models.Model):
    _name = 'ks.email.entry'
    _rec_name = 'email'
    _description = "Email Entry"

    ks_shopify_instance = fields.Many2one("ks.shopify.connector.instance", string="Instance", ondelete='cascade')
