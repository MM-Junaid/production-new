...
class KsDashboard(models.Model):
    ...
    def _fetch_graph_data(self):
        ...
        # # For Variants
        # ks_variant_new_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'product_product'), ('state', '=', 'new')])
        # ks_variant_progress_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'product_product'), ('state', '=', 'progress')])
        # ks_variant_done_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'product_product'), ('state', '=', 'done')])
        # ks_variant_failed_count = self.env['ks.shopify.queue.jobs'].search_count(
        ...
        # # For Delivery
        # ks_delivery_new_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'delivery'), ('state', '=', 'new')])
        # ks_delivery_progress_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'delivery'), ('state', '=', 'progress')])
        # ks_delivery_done_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'delivery'), ('state', '=', 'done')])
        # ks_delivery_failed_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'delivery'), ('state', '=', 'failed')])

        # # For Invoices
        # ks_invoice_new_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'invoice'), ('state', '=', 'new')])
        # ks_invoice_progress_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'invoice'), ('state', '=', 'progress')])
        # ks_invoice_done_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'invoice'), ('state', '=', 'done')])
        # ks_invoice_failed_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'invoice'), ('state', '=', 'failed')])

        # # For Refunds
        # ks_refund_new_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'refund'), ('state', '=', 'new')])
        # ks_refund_progress_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'refund'), ('state', '=', 'progress')])
        # ks_refund_done_count = self.env['ks.shopify.queue.jobs'].search_count(
        #     [('ks_model', '=', 'refund'), ('state', '=', 'done')])
        # ks_refund_failed_count = self.env['ks.shopify.queue.jobs'].search_count(
    ...
    def _compute_count(self, domain=False):
        ...
        customer_domain = [('ks_type', '=', 'customer')] if domain else []
            ...
            if domain_parse:
                for i in range(len(domain_parse)):
                    if isinstance(domain_parse[i], tuple):
                        ...
                        temp = tuple(temp)
                        domain_parse[i] = temp
        action = self.env.ref('ks_shopify.action_ks_shopify_connector_instance').read()[0]
    ...
    def get_ks_variants(self):
        action = self.env.ref('ks_shopify.action_ks_shopify_product_variants_').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_attributes(self):
        action = self.env.ref('ks_shopify.action_ks_shopify_product_attribute').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_refunds(self):
        action = self.env.ref('ks_shopify.action_shopify_refund').read()[0]
        action['domain'] = [('move_type', '=', 'out_refund'), ('ks_shopify_order_uni_id', 'not in', ['0',0,False])]
        return action

    def get_ks_categories(self):
        action = self.env.ref('ks_shopify.ks_shopify_collection_actions').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action
    ...
    def get_inventory(self):
        action = self.env.ref('ks_shopify.action_shopify_inventory_adjustments').read()[0]
